...
public class HlsChunkSource implements HlsTrackSelector.Output {
  ...
  public void getChunkOperation(TsChunk previousTsChunk, long playbackPositionUs,
    ...
    if (live) {
      if (previousTsChunk == null) {
        if (!firstLiveEventChunkSelected || playbackPositionUs > getLiveEdgeUs(nextVariantIndex)) {
  ...
  /**
   * This can be used to adjust the position for live window seeking and ensures that
   * the returned position will not be behind the live window.
   *
   * @param positionUs the target position
   * @return The new position
   */
  public long limitToliveEdge(long positionUs) {
    if(!live) return positionUs;
    int nextVariantIndex;
    if (adaptiveMode == ADAPTIVE_MODE_NONE) {
      nextVariantIndex = selectedVariantIndex;
    } else {
      nextVariantIndex = getNextVariantIndex(null, positionUs);
    }
    long liveEdgeUs = getLiveEdgeUs(nextVariantIndex);
    return liveEdgeUs < positionUs ? liveEdgeUs : positionUs;
  }
